...
type BackendEventCb = (payload: { run_id: string; event: unknown }) => void;
...
function installFakeBackend() {
  ...
  const backend = {
    call: vi.fn(async (method: string, params: unknown) => {
      ...
      if (method === 'run.create') {
        ...
        return {
          ok: true as const,
          result: { run_id: `run-${created}`, task_id: `btask-${created}`, status: 'running' },
        };
      }
  ...
  return {
    ...
    emit(event: RunEvent) {
      eventCbs.forEach((cb) => cb({ run_id: event.run_id, event }));
    },
...
describe('并发跑两个需求', () => {
  ...
  it('第二个需求不会把第一个卡死：两个 run 各自独立推进', async () => {
    ...
    // ── 两个需求先后提交 ──
    useDemoStore.getState().createTask('实现贪吃蛇');
    await vi.waitFor(() => expect(useDemoStore.getState().tasks[0].contractRunId).toBe('run-1'));

    useDemoStore.getState().createTask('实现俄罗斯方块');
  ...
  it('别的项目还有 run 在跑时，跨项目提交被拒 —— 绝不重启后端（重启会杀掉那个 agent）', async () => {
    ...
    expect(useDemoStore.getState().createTask('实现贪吃蛇')).toEqual({ ok: true });
    await vi.waitFor(() => expect(useDemoStore.getState().tasks[0].contractRunId).toBe('run-1'));
    fake.emit(evt('run-1', 'task.created', { spec: '实现贪吃蛇' }));
    ...
    const result = useDemoStore.getState().createTask('实现俄罗斯方块');
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.error).toContain('实现贪吃蛇');
    expect(fake.backend.configure.mock.calls.length).toBe(configureCallsBefore);
    expect(fake.calls('run.create')).toHaveLength(1); // 只有 P1 那一次
  ...
  it('后端事件永远不会启动终端进程（R3/I5 硬红线）', async () => {
    ...
    useDemoStore.getState().resetDemo();
    useDemoStore.getState().createProject('P1', '项目一');
    useDemoStore.getState().createTask('写一个贪吃蛇');
    await vi.waitFor(() => expect(useDemoStore.getState().tasks[0].contractRunId).toBe('run-1'));
