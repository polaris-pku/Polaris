...
export function AuthBanner({ onConfigure }: { onConfigure: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onConfigure}
      className="group flex w-full items-center gap-2 border-b border-human/30 bg-human/10 px-5 py-2 text-left transition-colors hover:bg-human/15"
    >
      <KeyRound className="h-4 w-4 shrink-0 text-human-soft" />
      <span className="min-w-0 text-body text-human-soft">
        {incomplete
      ...
      <span className="ml-auto flex shrink-0 items-center gap-1 rounded-full border border-human/40 bg-human/15 px-3 py-1 text-body text-human-soft group-hover:border-human/60">
