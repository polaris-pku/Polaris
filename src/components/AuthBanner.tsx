...
export function AuthBanner({ onConfigure }: { onConfigure: () => void }) {
  ...
  const agentName = status.agents.find((a) => a.id === status.auth.agentId)?.name ?? 'Agent';
  ...
  return (
    ...
    >
      ...
      <span className="text-xs text-human-soft">
        <strong className="font-semibold">还差一步：配置 {agentName} 的 API key</strong>
        <span className="ml-2 text-human-soft/70">
          {agentName} 已随应用安装，无需另外安装；填入 key 后 agent 才能真正干活。
        </span>
      </span>
